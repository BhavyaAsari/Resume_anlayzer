...
class Affinda:
    ...
    def parse_resume(file_stream, filename="resume.pdf"):
        ...
        try:
            ...
            # Option 1: Send the actual PDF file (recommended)
            headers = {
                "Authorization": f"Bearer {AFFINDA_API_KEY}",
                # Don't set Content-Type, let requests handle it for file uploads
            }
            ...
            response = requests.post(Affinda.API_URL, headers=headers, files=files)
            
            # If file upload fails, try fallback method
            if response.status_code not in [200, 201]:
                print(f"⚠️ File upload failed ({response.status_code}), trying text fallback...")
                return Affinda._parse_with_text_fallback(file_stream, filename)
    ...
    @staticmethod
    def _parse_with_text_fallback(file_stream, filename):
        ...
        try:
            ...
            # Send as plain text (check Affinda docs for correct format)
            headers = {
                "Authorization": f"Bearer {AFFINDA_API_KEY}",
            ...
            response = requests.post(Affinda.API_URL, headers=headers, data=clean_text)
...
# Alternative simple function if you just want to extract text
def extract_text_fallback(file_stream):
    """Simple fallback function to extract text from PDF"""
    try:
        file_stream.seek(0)
        raw_text = extract_text(file_stream)
        clean_text = unidecode(raw_text)
